...
def update_checkpoint(checkpoint, losses, metrics, name="train"):
    for task_idx, (loss, metric) in enumerate(zip(losses, metrics)):
        checkpoint[task_idx][f"{name}_losses"].append(loss)
        checkpoint[task_idx][f"{name}_metrics"].append(metric)
