...
class LightGBM(GBDT):
    ...
    def objective(
        ...
        self.params = {
            ...
            "max_depth":
            trial.suggest_int("max_depth", 3, 5),
            "learning_rate":
            ...
            "min_data_in_leaf":
            trial.suggest_int("min_data_in_leaf", 1, 3),
        }
        ...
        try:
            ...
            # Note that for mse, score should be inf
            # return acc = 0 if min_data_in_leaf is too large 
            score = 0
    ...
    def cross_validation(self, tf_train: TensorFrame, tf_test: TensorFrame, seed: int):
        ...
        # define model
        estimator = lightgbm.LGBMClassifier(class_weight='balanced', num_threads=1, random_state=seed)
        
        # convert input tensorframe into pandas data frame
        train_x, train_y, cat_features = self._to_lightgbm_input(tf_train)
        test_x, test_y, _ = self._to_lightgbm_input(tf_test)
