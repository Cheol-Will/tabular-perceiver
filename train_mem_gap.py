...
from models import MemGlovalAvgPool
...
def train(
    ...
    for (tf, indicies) in tqdm(loader, desc=f'Epoch: {epoch}'):
        ...
        # pass index so that it does not retrieve itself
        pred = model(tf, indicies)
        if pred.size(1) == 1:
            pred = pred.view(-1, )
